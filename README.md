...
### Image 3: Export PowerBI Report to PDF and native support for Purview Sensitivity Labels
This screenshot shows the added capability of export the report as pdf. The pdf will inherit the sensitvity label of the report. This label is also displayed natively in the embedding.
<img width="1492" height="987" alt="Screenshot 2026-01-21 181932" src="https://github.com/user-attachments/assets/05f6baed-1aaf-4e08-a0d8-a7593a44a8d6" />
...
> **Tip:** These commands assume the PowerShell or Bash working directory is the repo root (`Powerbi-js-embedded-demo`). All resources are billed—clean them up when you finish testing.

1. **Configure production IDs**
   - Update [samples/org-owns-data/src/public/config.js](samples/org-owns-data/src/public/config.js) with the Entra App Registration ID, tenant ID, workspace ID, and report ID that match your production environment.
   - Keep `redirectUri` in [samples/org-owns-data/src/public/authConfig.js](samples/org-owns-data/src/public/authConfig.js) set to `window.location.origin`; in the Entra portal add both `http://localhost:3000` and the final HTTPS URL (for example, `https://powerbi-viewer.azurewebsites.net`) as SPA redirect URIs.

2. **Provision Azure resources (replace names as needed)**

   ```bash
   az group create -n rg_powerbi_demo -l swedencentral
   az appservice plan create -n asp-powerbi-demo -g rg_powerbi_demo --sku S1 --is-linux
   az webapp create -n powerbi-viewer -g rg_powerbi_demo -p asp-powerbi-demo --runtime "NODE|18-lts"
   ```
...
3. **Create a deployment ZIP from the sample folder**
   ...
   ```powershell
   Compress-Archive -Path samples/org-owns-data/* -DestinationPath powerbi-viewer.zip -Force
   ```

4. **Deploy with ZIP deploy**

   ```bash
   az webapp deploy -g rg_powerbi_demo -n powerbi-viewer --src-path powerbi-viewer.zip --type zip
   ```

   - If you prefer CI/CD, point GitHub Actions (or Azure DevOps) at `samples/org-owns-data`, skip any build step, and run `npm start` as the startup command.

5. **Verify and update Azure AD redirect URIs**
   - Navigate to `https://powerbi-viewer.azurewebsites.net/api/health` to confirm the Express server responds with JSON.
   - Open the portal URL, sign in, and ensure the embedded report renders. If you see the generic “Your web app is running” page, confirm the runtime is set to `NODE|18-lts` via `az webapp config show --query linuxFxVersion` and redeploy.

6. **Clean up when finished**
   - Delete the resource group (`az group delete -n rg_powerbi_demo --yes --no-wait`) to avoid ongoing charges, or keep it and reuse the plan/web app for future updates.
...
- Stuck on sign-in or token errors:
  - Check App Registration redirect URI matches your origin (e.g., `http://localhost:3000` and/or  `https://thedeployed.webapp.com` ).
...
5. Redirect URI:
    ...
    - Enter: `http://localhost:3000` (you can add several, e.g. add the url of the deployed webapp`https://thedeployed.webapp.com` )
6. Click **Register**.

**After registration:**

1. Go to your app registration's **Overview** page.
    - Copy the **Application (client) ID** and **Directory (tenant) ID** for use in `.env` or `src/public/config.js`.
    - **Application (client) ID**: Copy this value for `AAD_CLIENT_ID` in `.env` or `src/public/config.js`.
    - **Directory (tenant) ID**: Copy this value for `AAD_TENANT_ID` in `.env` or `src/public/config.js`.
...
4. Copy these IDs for `POWER_BI_WORKSPACE_ID` and `POWER_BI_REPORT_ID` in `.env` or `src/public/config.js`.
